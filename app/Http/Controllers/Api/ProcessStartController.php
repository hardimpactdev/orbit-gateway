...
#[RequiresPermission('process:start', servingNode: ServingNode::AppOwning)]
final class ProcessStartController implements Loggable
{
    ...
    public function __invoke(Request $request, StartProcesses $startProcesses): JsonResponse
    {
        /** @var mixed $caller */
        $caller = $request->user();
        ...
        $authorization = $this->authorizeProcessAccess($caller, $context->node, 'process:start');
        ...
        try {
            $result = $startProcesses->handle($context, $name);
        } catch (GatewayApiException $e) {
            return $this->error($e->errorCode() ?? 'validation_failed', $e->getMessage(), $e->errorMeta(), $e->errorData(), $this->statusFor($e));
        }
    ...
    {
        return 'api:POST /processes/start';
    }
